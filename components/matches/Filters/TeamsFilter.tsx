...
export const TeamsFilter = ({ teams }: TeamsFilterProps) => {
  ...
  return (
    <div className="rounded-sm p-4 text-black dark:text-white text-sm bg-[#F1F1F5] dark:bg-[#191921] space-y-3 flex flex-col w-full">
      ...
      <VisibleTeamsList
        teams={visibleTeams}
        onTeamClick={handleVisibleTeamClick}
      />

      <SearchByTeamFilter
        availableTeams={searchFilterOptions}
        onSearchByFilterPick={handleSearchByFilterPick}
      />
    </div>
  );
};
...
const SearchByTeamFilter = ({
  availableTeams,
  onSearchByFilterPick,
}: SearchByTeamFilterProps) => {
  ...
  for (const team of availableTeams) {
    ...
    if (!comboBoxTeamsData.some((object) => object.value === gameName)) {
      ...
      comboBoxTeamsData
        .find((object) => object.value === gameName)
  ...
  const handleTeamSelect = (gameName: string, teamName: string) => {
    const selectedTeam = availableTeams.find(
      (team) => team.game.name === gameName && team.name === teamName
    );
              ...
              <ComboboxCollection>
                {(item) => (
                  <ComboboxItem
                    key={item}
                    value={item}
                    onClick={() => handleTeamSelect(group.value, item)}
                  >
                    {item}
                  </ComboboxItem>
                )}
